...
def make_dot(studio, var, params=None):
    ...
    def add_nodes(var):
        if var not in seen:
            if torch.is_tensor(var):
                ...
                studio.AddNodeGraphDef(str(id(var)), str(id(var)))
            elif hasattr(var, 'variable'):
                u = var.variable
                name = param_map[id(u)] if params is not None else ''
                node_name = '%s\n %s' % (name, size_to_str(u.size()))
                studio.AddNodeGraphDef(str(id(var)), node_name)
            elif var in output_nodes:
                studio.AddNodeGraphDef(str(id(var)), str(type(var).__name__))
            else:
                studio.AddNodeGraphDef(str(id(var)), str(type(var).__name__))
            seen.add(var)
            if hasattr(var, 'next_functions'):
                for u in var.next_functions:
                    if u[0] is not None:
                        studio.AddNodeInputGraphDef(str(id(u[0])))
                        add_nodes(u[0])
            if hasattr(var, 'saved_tensors'):
                for t in var.saved_tensors:
                    studio.AddNodeInputGraphDef(str(id(t)))
                    add_nodes(t)
...
def make_dot_from_trace(studio, trace):
    ...
    for node in list_of_nodes:
        ...
        if node.inputs:
            for inp in node.inputs:
                studio.AddNodeInputGraphDef(inp)