...
flags.DEFINE_string("data_path", dir_path + "\\data\\" ,
...
class SampleInput(object):
    ...
    def __init__(self, config, name=None):
        ...
        self.num_steps = num_steps = config.num_steps
        self.epoch_size = 1
        self.targets = tf.zeros([batch_size, num_steps], tf.int32)
...
def do_sample(session, model, data, num_samples):
    ...
    while k < num_samples:
        feed_dict = {}
        feed_dict[model.input.input_data] = [[samples[-1]]]
        for layer_num, (c, h) in enumerate(model.initial_state):
            feed_dict[c] = state[layer_num].c
            feed_dict[h] = state[layer_num].h
        state, sample = session.run(fetches, feed_dict)
...
def main(_):
    ...
    bad_input = True
    while bad_input:
        seed_for_sample = studio.GetInputText("Enter your sample prefix")
        seed_for_sample = seed_for_sample.split()
        bad_input = False
        for x in seed_for_sample:
            if x not in word_to_id:
                print (x + " not in the dictionay")
                bad_input = True
    print("input words:", seed_for_sample)
    ids = [word_to_id[x] for x in seed_for_sample]
    print( "input word id:", ids)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Test"):
            test_input = SampleInput(config=eval_config, name="SampleInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
        ...
        with sv.managed_session(config=config_proto) as session:
            ...
            for i in range(config.max_max_epoch):
                ...
                print("Sample: %s" % pretty_print(do_sample(session, mtest, [word_to_id[word] for word in seed_for_sample],
                                                            max(50 * (len(seed_for_sample) + 1), 10)), id_2_word))
